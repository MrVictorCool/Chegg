...
public abstract class Egg {
    ...
    public void initializeEgg(int xCoordinate, int yCoordinate, int worldX, int worldY, Team team) {
        ...
        this.worldY = worldY;
        this.team = team;
        try {setImage(ImageIO.read(getClass().getResourceAsStream(imagePath)));}
        catch (IOException e) {e.printStackTrace();}
    }

    public void initializeEgg(Vector2i v2, Vector2i world, Team team) {
        initializeEgg(v2.x, v2.y, world.x, world.y, team);
    }
    ...
    /**
     * <p> Finds as many moves as possible while repeating a step {@code Vector2i} and checking in each if it collided. It <b> does not </b> account for in-between tiles.
     * This allow for an extra argument {@code repetitions} which determins the max amount of steps.
     * </p>
     * <ul> <p> <b> The expected behaviour is as follows: </b> </p> </ul>
     * 
     * <ul>
    ...
    protected Vector2i[] calculateMovesInVector(Vector2i step, Board board, int repetitions) {
        ...
        while (target.isOnBound(board)) {
            if (repetitions < 1) {return moves.toArray(new Vector2i[0]);}
            repetitions--;
            if (board.getEggAt(target) != null) {
                if (!board.getEggAt(target).isSameTeamAs(this)) {
                    moves.add(target);
